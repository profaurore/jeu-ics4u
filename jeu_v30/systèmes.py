...
def lancer_niveau(état):
    ...
    for données in état.niveau.entités_init:
        type_entité = données['type']

        if type_entité == 'téléportation':
            créer_téléportation(état.ecs, données['x'], données['y'], données['cx'], données['cy'])
...
def appliquer_interactions_entités(état):
    c = état.ecs.composants

    for entité_téléportation, téléportation in c[Téléportation].items():
        if entité_téléportation not in c[Position]:
            continue
        pos_téléportation = c[Position][entité_téléportation]
        
        for entité_joueur in c[Joueur].keys():
            if entité_joueur not in c[Position]:
                continue
            pos_joueur = c[Position][entité_joueur]

            if pos_téléportation.x == pos_joueur.x and pos_téléportation.y == pos_joueur.y:
                pos_joueur.x = téléportation.cx
                pos_joueur.y = téléportation.cy
                arriver_centre_tuile(état, entité_joueur, pos_joueur.x, pos_joueur.y)
