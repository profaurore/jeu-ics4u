...
def main():
    ...
    état.niveau = charger_monde(C.DÉF_MONDE_SOURCE)

    état.ecs = ECS()
    ajouter_type_composant(état.ecs, Débogage, Position, Sprite, Force, MouvementGrillage, Joueur, Téléportation)
    ajouter_système(état.ecs, déplacer_carré_débogage, appliquer_effets_tuiles, appliquer_mouvement_grillage, appliquer_forces, appliquer_interactions_entités)
    créer_carré_débogage(état.ecs)
    état.joueur = créer_joueur(état.ecs)
    lancer_niveau(état)
